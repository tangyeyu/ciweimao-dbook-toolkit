...
> ⚠️ **先看这句，别误会它**：本工具箱是 **「段评采集 + 打包 + 安卓阅读器」**，**不是解密 / 格式转换 / 绕过付费墙的工具**。
> - 段评是**对公众可见的读者评论**（游客免登录即可读），本项目只把它们抓下来放进手机里，像原版那样点段看评论。
> - **明确不碰正文付费墙**：刺猬猫付费章段评虽然能抓，但**正文**若未购买，接口直接返回「图片化 / 需订阅」，本工具承认拿不到、也**不去绕**；起点 VIP 权限同样在服务端。
> - 硬骨头（画重点）在段评采集本身：App 接口逆向、瑞数 WAF 双风控对抗、章节索引校准、滑块可见性误判——这些才是反复迭代的地方。
...
平台的段评接口大多没对外文档，还会随版本变动，这几个才是真正的硬骨头：

| 难点 | 解决方案 |
|---|---|
| 刺猬猫 App 接口加密 | 完整逆向：`AES-256-CBC` 响应解密、`HMAC-SHA256` 请求签名、`RSA` 登录、`geetest` 版本墙对抗 |
| 起点瑞数 WAF | **实例级拉黑**（浏览器指纹）靠全新 profile 轮换干净实例；**IP 级风控**只能等冷却 + 保守并发（2 / 800ms） |
| 起点真实浏览器 | Node 直连被 WAF 挑战页 202 拦截，改用 CDP 借用真实 Chrome 上下文同源 fetch（游客免登录即可抓全程段落评） |
| 章节索引错位 | EPUB 章序 vs 平台 `chapter_index` 存在 `+6` 偏移，用**标题滑窗归一化匹配**做整书精校准 |
| 滑块验证 | 平台把 turing 滑块藏在 `y:-1000000` 隐藏层，必须用**视口可见性**判定（否则永远误判「有滑块」）；检测到可真弹窗时用像素差分 + 轨迹模拟自动拖过 |
| 段评段落对齐 | 平台段落号与本地正文行号不一致时，按 `max(P, L-1)` 线性映射，段落级错位归零 |
...
- 本工具采集的是**公开可见的段评（读者评论）**，不含任何付费正文内容；正文请通过正规渠道购买。
- 项目仅用于**个人学习、兴趣与技术交流**，请勿用于商业用途或大规模抓取。
- 爬取请保持克制，滥用可能导致账号风控。

...
刺猬猫（欢乐书客）的段评是小说阅读的灵魂——读者在每段话下面插科打诨、预测剧情、玩梗接龙。但 Web 端段评早已下线，App 端也不支持导出。

这个工具箱帮你：

1. **抓取** 任意刺猬猫小说的全部段评（含付费章节，免登录只读也能抓）
2. **打包** 成自研 `.dbook` 单文件格式（正文 + 段评一体化）
3. **阅读** 在安卓「段评书架」App 里像原版一样点段看评论，还支持 **音量键翻页**

```
刺猬猫 App 接口 ──爬虫面板──▶ ciweimao_data/ ──打包面板──▶ .dbook ──导入──▶ 段评书架 App
...
| 工具 | 作用 | 入口 |
|---|---|---|
| `ciweimao_vis_crawler.mjs` | 段评可视化爬虫面板 | `http://127.0.0.1:8788` |
...
- 爬取请保持克制，滥用可能导致账号风控。
- 段评及正文版权归原作者/平台所有；本项目与刺猬猫官方无任何关联。
